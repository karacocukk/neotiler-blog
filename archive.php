...
<div class="container mx-auto px-4 max-w-[1200px] pt-8 pb-16">
	...
	<?php if (have_posts()): ?>
		...
		<header class="mb-12 border-b border-slate-200 dark:border-slate-800 pb-8">
			<?php
			the_archive_title('<h1 class="text-3xl md:text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-3">', '</h1>');
			the_archive_description('<p class="text-slate-500 dark:text-slate-400 text-lg max-w-2xl">', '</p>');
			?>
		</header>
	...
	<?php else: ?>
		...
		<div class="text-center py-20">
			<h1 class="text-3xl font-black text-slate-900 dark:text-white mb-4">
				<?php esc_html_e('Nothing Found', 'neotiler-blog'); ?></h1>
			<p class="text-slate-500 dark:text-slate-400 text-lg mb-8">
				<?php esc_html_e('It seems we can\'t find what you\'re looking for.', 'neotiler-blog'); ?></p>
			<a href="<?php echo esc_url(home_url('/')); ?>"
				class="inline-block bg-primary text-white font-bold px-6 py-3 hover:bg-blue-700 transition-colors">
				<?php esc_html_e('Go Home', 'neotiler-blog'); ?>
			</a>
		</div>
